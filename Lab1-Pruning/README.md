...
According to the result, we can draw the relationship for the conv and fc layers easily.
...
### Channel Pruning
By using Frobenius-norm(L2-norm), sort the weight for every channel of each origin model's layer. For each layer of this model, we must preserve the channel which has high priority.
